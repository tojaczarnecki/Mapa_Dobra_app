...
export default async function AdminImportBatchPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
    ...
    include: {
      candidates: {
        ...
        include: {
          ...
          createdPlace: { select: { id: true, name: true } },
  ...
  if (!batch) notFound();
  const counts = await prisma.importCandidate.groupBy({ by: ["status"], where: { importBatchId: id }, _count: { _all: true } });
      ...
      {batch.candidates.length ? <ol className="space-y-2">{batch.candidates.map((candidate) => (
        <li key={candidate.id} className="rounded-lg border border-border bg-white p-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            {candidate.createdPlace ? <Link href={`/admin/miejsca/${candidate.createdPlace.id}`} className="inline-flex min-h-11 items-center rounded-lg border border-border px-3 text-sm font-bold text-brand-strong hover:bg-brand-soft">Otwórz szkic</Link> : candidate.matchedPlace ? <Link href={`/admin/miejsca/${candidate.matchedPlace.id}`} className="inline-flex min-h-11 items-center rounded-lg border border-border px-3 text-sm font-bold text-brand-strong hover:bg-brand-soft">Możliwy rekord: {candidate.matchedPlace.recordKind}</Link> : null}
          </div>
