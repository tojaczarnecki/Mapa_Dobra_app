...
export default async function EditOrganizationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <OrganizationForm initialData={{ id, name: organization.name, description: organization.description ?? "", phone: organization.phone ?? "", email: organization.email ?? "", website: organization.website ?? "" }} />
    </div>
  );
}
