...
import { isGusConfigured } from "@/lib/organizations/gus-client";
...
export default async function EditOrganizationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <OrganizationForm gusConfigured={isGusConfigured()} initialData={{ id, name: organization.name, description: organization.description ?? "", phone: organization.phone ?? "", email: organization.email ?? "", website: organization.website ?? "", nip: organization.nip ?? "", regon: organization.regon ?? "", krs: organization.krs ?? "", legalForm: organization.legalForm ?? "" }} />
    </div>
  );
}
