...
import { UndoCandidateResolutionForm } from "@/components/admin/verification/undo-candidate-resolution-form";
...
import { scheduleFromRows, weekdayLabels } from "@/lib/places/opening-hours";
...
import { AdminImpactSummary, AdminWorkflowSteps } from "@/components/admin/admin-ui";
import { placeStatusLabels, recordKindLabels } from "@/lib/places/constants";

const uuidPattern = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/iu;
const candidateStatusLabels: Record<string, string> = { IMPORT_READY: "Gotowy do utworzenia szkicu", MATCH_EXISTING: "Dopasowany do istniejącego miejsca", REQUIRES_REVIEW: "Wymaga decyzji", IMPORTED: "Utworzono szkic", SKIPPED: "Pominięty" };
const verificationBlockerLabels: Record<string, string> = {
  "Zatwierdzona lokalizacja": "Brak zatwierdzonej lokalizacji",
  "Aktualna weryfikacja": "Brak aktualnej weryfikacji",
  "Godziny": "Brak potwierdzonych godzin",
  "Warunki": "Brak potwierdzonych warunków",
  "Kategoria": "Brak aktywnej kategorii",
  "Konflikty": "Nierozwiązany konflikt danych",
  "Krytyczne dane": "Brak wymaganych danych kontaktowych",
  "Grupa noclegu": "Brak grupy docelowej dla noclegu",
};
...
export default async function VerificationDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!candidate || (!candidate.queueStatus && !isSpreadsheetPlaceReviewCandidate(candidate))) notFound();
  ...
  return (
    <div className="space-y-5">
      ...
      <AdminWorkflowSteps steps={[
        { label: "Sprawdź dane miejsca", detail: "Adres, kontakt, zakres pomocy i godziny.", state: complete.readyToPublish ? "done" : "current" },
        { label: "Uzupełnij braki", detail: complete.readyToPublish ? "Nie ma blokujących braków." : "Zobacz konkretne braki w podsumowaniu.", state: complete.readyToPublish ? "done" : "blocked" },
        { label: "Podejmij decyzję", detail: complete.readyToPublish ? "Możesz zakończyć weryfikację." : "Najpierw usuń blokujące braki.", state: complete.readyToPublish ? "current" : "next" },
      ]} />
      {!complete.readyToPublish ? <AdminImpactSummary title="Co blokuje zakończenie weryfikacji"><ul className="list-disc pl-5">{complete.checks.filter((check) => check.state === "missing").map((check) => <li key={check.key}>{verificationBlockerLabels[check.label] ?? `Brak: ${check.label.toLocaleLowerCase("pl-PL")}`}</li>)}</ul></AdminImpactSummary> : null}
      <header className="rounded-lg border border-border bg-white p-4 sm:p-5"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm font-bold text-brand-strong">Nowe miejsce z importu</p><h1 className="mt-1 text-2xl font-bold sm:text-3xl">{place.name}</h1><p className="mt-2 text-sm text-muted-foreground">{place.addressLine}</p><div className="mt-3 flex flex-wrap gap-2"><VerificationStatusBadge status={place.verificationQueueStatus!} /><span className="inline-flex min-h-7 items-center rounded-full border border-border px-2.5 py-1 text-xs font-bold">{recordKindLabels[place.recordKind]}</span><span className="inline-flex min-h-7 items-center rounded-full border border-border px-2.5 py-1 text-xs font-bold">{placeStatusLabels[place.publicationStatus]}</span></div><div className="mt-3 flex flex-wrap gap-2 text-xs font-bold"><WorkflowPill complete={place.latitude !== null && place.longitude !== null && place.locationSource !== null} label="Lokalizacja" /><WorkflowPill complete={place.verificationStatus === "VERIFIED"} label="Weryfikacja" />{place.verificationQueueStatus === "CONTACT_REQUIRED" ? <span className="inline-flex min-h-7 items-center rounded-full border border-[#d7a548] bg-[#fff4d8] px-2.5 py-1 text-[#684500]">Kontakt wymagany</span> : null}<WorkflowPill complete={complete.readyToPublish} label="Gotowe" /></div></div>{place.verificationQueueStatus === "PENDING" ? <form action={startPlaceVerification.bind(null, place.id)}><button className="inline-flex min-h-11 items-center gap-2 rounded-lg border border-brand px-4 text-sm font-bold text-brand-strong hover:bg-brand-soft"><Play aria-hidden="true" size={17} />Rozpocznij weryfikację</button></form> : null}</div></header>
...
async function CandidateVerification({ candidate, navigation }: { candidate: NonNullable<Awaited<ReturnType<typeof prisma.importCandidate.findUnique>>> & Record<string, unknown>; navigation: Awaited<ReturnType<typeof getVerificationNavigation>> }) {
  ...
  return (
    <div className="space-y-5">
      <VerificationNav {...navigation} />
      <header className="rounded-lg border border-border bg-white p-4 sm:p-5"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm font-bold text-urgent">{typed.resolution ? "Rozstrzygnięty kandydat importowy" : "Kandydat wymagający ręcznej decyzji"}</p><h1 className="mt-1 text-2xl font-bold sm:text-3xl">{typed.proposedName}</h1><p className="mt-2 text-sm text-muted-foreground">{typed.proposedAddress ?? "Brak stałego adresu"}</p><div className="mt-3 flex flex-wrap gap-2"><VerificationStatusBadge status={typed.queueStatus!} /><span className="inline-flex min-h-7 items-center rounded-full border border-border px-2.5 py-1 text-xs font-bold">{candidateStatusLabels[typed.status] ?? "Status do sprawdzenia"}</span></div></div>{typed.queueStatus === "PENDING" ? <form action={startCandidateVerification.bind(null, typed.id)}><button className="inline-flex min-h-11 items-center gap-2 rounded-lg border border-brand px-4 text-sm font-bold text-brand-strong hover:bg-brand-soft"><Play aria-hidden="true" size={17} />Rozpocznij analizę</button></form> : null}</div></header>
      <div className="grid gap-5 xl:grid-cols-2"><SourceData sources={typed.sources.map((item) => item.sourceEntry)} sourceUrl={typed.importBatch.sourceUrl} /><section className="rounded-lg border border-border bg-white p-4 sm:p-5"><p className="text-xs font-bold uppercase text-brand-strong">Propozycja importera</p><h2 className="mt-1 text-xl font-bold">Dane robocze kandydata</h2><dl className="mt-3 divide-y divide-border text-sm"><Row label="Nazwa" value={typed.proposedName} /><Row label="Organizacja" value={typed.proposedOrganizationName ?? "Brak danych"} /><Row label="Adres" value={typed.proposedAddress ?? "Brak stałego adresu"} /><Row label="Telefon" value={typed.proposedPhone ?? "Brak danych"} /><Row label="Kategorie" value={typed.categorySlugs.join(" · ") || "Brak klasyfikacji"} /><Row label="Główna kategoria" value={typed.primaryCategorySlug ?? "Wymaga decyzji"} /><Row label="Godziny źródłowe" value={typeof proposed.rawOpeningHours === "string" ? proposed.rawOpeningHours : "Brak danych"} /></dl>{typed.reviewReasons.length ? <ul className="mt-3 space-y-1 rounded-md border border-urgent/25 bg-urgent-soft/30 p-3 text-sm">{typed.reviewReasons.map((reason) => <li key={reason}>• {importIssueLabel(reason)}</li>)}</ul> : null}</section></div>
      <section className="rounded-lg border border-border bg-white p-4 sm:p-5"><h2 className="text-xl font-bold">Porównanie możliwych rekordów</h2><p className="mt-1 text-sm text-muted-foreground">Dopasowanie jest wyliczane ponownie na podstawie aktualnych danych miejsca.</p>{comparison.liveMatch ? <p className="mt-2 text-sm font-semibold text-muted-foreground">{comparison.liveMatch.classification === "EXACT_MATCH" ? "Aktualne dokładne dopasowanie" : comparison.liveMatch.classification === "POSSIBLE_MATCH" ? "Aktualne możliwe dopasowanie" : "Nie znaleziono już aktualnego konfliktu"}</p> : null}{comparison.suggestions.length ? <div className="mt-3 grid gap-2">{comparison.suggestions.map((place) => <article key={place.id} className="rounded-lg border border-border p-3"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-bold">{place.name}</h3><p className="mt-1 text-sm">{place.addressLine}</p><p className="mt-1 text-xs text-muted-foreground">{place.phone ?? "Brak telefonu"} · {place.organization?.name ?? "Brak organizacji"} · {place.categories.map((item) => item.category.name).join(" · ")}</p>{place.matchReasons?.length ? <p className="mt-2 text-xs text-muted-foreground">{place.matchReasons.map((reason) => importIssueLabel(reason)).join(" · ")}</p> : null}</div><Link href={`/admin/miejsca/${place.id}`} className="inline-flex min-h-11 items-center rounded-lg border border-border px-3 text-sm font-bold text-brand-strong hover:bg-brand-soft">Otwórz miejsce</Link></div></article>)}</div> : <p className="mt-3 text-sm text-muted-foreground">{comparison.liveMatch ? "Nie znaleziono już aktualnego konfliktu." : "Nie znaleziono wiarygodnego podobnego miejsca w bazie."}</p>}{comparison.relatedCandidates.length ? <div className="mt-4"><h3 className="text-sm font-bold">Podobni kandydaci w tej paczce</h3><ul className="mt-2 space-y-1 text-sm">{comparison.relatedCandidates.map((item) => <li key={item.id}><Link href={`/admin/weryfikacja/${item.id}`} className="font-semibold text-brand-strong hover:underline">{item.proposedName}</Link> · {item.proposedAddress ?? "brak adresu"}</li>)}</ul></div> : null}</section>
      {typed.createdPlace ? <div className="rounded-lg border border-brand/35 bg-brand-soft p-4"><strong>Utworzony szkic:</strong> <Link href={`/admin/miejsca/${typed.createdPlace.id}`} className="font-bold text-brand-strong hover:underline">{typed.createdPlace.name}</Link></div> : null}
      {typed.resolution ? <section className="rounded-lg border border-brand/30 bg-brand-soft/35 p-4 sm:p-5"><h2 className="text-xl font-bold">Decyzja została zapisana</h2><dl className="mt-3 divide-y divide-brand/20 text-sm"><Row label="Rozstrzygnięcie" value={typed.resolution === "SAME_PLACE" ? "To to samo miejsce" : typed.resolution === "DIFFERENT_PLACE" ? "To inna placówka" : "Pominięto"} /><Row label="Administrator" value={typed.resolvedBy?.displayName ?? "Administrator"} /><Row label="Data" value={typed.resolvedAt ? new Intl.DateTimeFormat("pl-PL", { dateStyle: "medium", timeStyle: "short" }).format(typed.resolvedAt) : "Brak daty"} /><Row label="Notatka" value={typed.resolutionNote ?? "Bez notatki"} /></dl>{typed.resolution === "DIFFERENT_PLACE" && typed.createdPlaceId ? <p className="mt-3 text-sm text-muted-foreground">Utworzono szkic miejsca. Cofnięcie tej decyzji wymaga obsługi utworzonego szkicu.</p> : null}{typed.resolution === "SAME_PLACE" || typed.resolution === "SKIPPED" ? <UndoCandidateResolutionForm candidateId={typed.id} resolution={typed.resolution} /> : null}<p className="mt-3 text-sm text-muted-foreground">Dane źródłowe i rozstrzygnięcie pozostają w historii. Decyzja nie publikuje miejsca.</p></section> : spreadsheetPlaceReview && comparison.liveMatch?.classification === "NO_MATCH" ? <section className="rounded-lg border border-border bg-white p-4 sm:p-5"><h2 className="text-xl font-bold">Nie znaleziono już aktualnego konfliktu</h2><p className="mt-2 text-sm text-muted-foreground">Dane miejsc w bazie zmieniły się od czasu skierowania tego rekordu do weryfikacji. Nie zapisano żadnej decyzji.</p><Link href="/admin/weryfikacja" className="mt-3 inline-flex min-h-11 items-center rounded-lg border border-border px-3 text-sm font-bold text-brand-strong hover:bg-brand-soft">Wróć do kolejki</Link></section> : <CandidateResolutionPanel candidateId={typed.id} importBatchId={typed.importBatch.id} spreadsheetPlaceReview={spreadsheetPlaceReview} effectiveCategory={spreadsheetPlaceReview ? effectiveCategorySummary(typed, options.categories) : null} initial={{ name: typed.proposedName, address: typed.proposedAddress ?? "", categorySlugs: typed.categorySlugs.filter((slug) => options.categories.some((item) => item.slug === slug && item.active)), primaryCategorySlug: typed.primaryCategorySlug ?? typed.categorySlugs[0] ?? "", organizationId, matchedPlaceId: typed.matchedPlaceId ?? "" }} categories={options.categories} organizations={options.organizations} places={(spreadsheetPlaceReview ? comparison.suggestions : comparison.allPlaces).map((item) => ({ id: item.id, name: item.name, addressLine: item.addressLine }))} />}
    </div>
  );
}

async function getCandidateWithRelations(id: string) {
  return prisma.importCandidate.findUnique({ where: { id }, include: { importBatch: true, sources: { include: { sourceEntry: true } }, matchedPlace: true, createdPlace: { select: { id: true, name: true } }, resolvedBy: { select: { displayName: true } }, categoryDecision: { select: { primaryCategoryId: true, categories: { orderBy: [{ sortOrder: "asc" }, { categoryId: "asc" }], select: { categoryId: true } } } } } });
}

function effectiveCategorySummary(candidate: Awaited<ReturnType<typeof getCandidateWithRelations>>, categories: Array<{ id: string; slug: string; name: string; active: boolean }>) {
  if (!candidate) return { primary: null, selected: [], review: true };
  const decision = candidate.categoryDecision;
  if (decision) {
    const selected = decision.categories.map((item) => categories.find((category) => category.id === item.categoryId)?.name ?? "Nieaktywna kategoria");
    return { primary: categories.find((category) => category.id === decision.primaryCategoryId)?.name ?? "Nieaktywna kategoria", selected, review: false };
  }
  const selected = candidate.categorySlugs.map((slug) => categories.find((category) => category.slug === slug)?.name ?? slug);
  return { primary: candidate.primaryCategorySlug ? categories.find((category) => category.slug === candidate.primaryCategorySlug)?.name ?? candidate.primaryCategorySlug : null, selected, review: !candidate.primaryCategorySlug };
}
...
function SourceData({ sources, sourceUrl }: { sources: Array<{ id: string; section: string; sourcePages: number[]; rawName: string; rawAddress: string | null; rawPhone: string | null; rawEmail: string | null; rawWebsite: string | null; rawOpeningHours: string | null; rawAdmissionHours: string | null; rawAssistanceDescription: string | null; rawText: string }>; sourceUrl: string }) {
  const isSpreadsheet = !sourceUrl;
  return <section className="rounded-lg border border-border bg-white p-4 sm:p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase text-urgent">Niezmienny zapis źródłowy</p><h2 className="mt-1 text-xl font-bold">{isSpreadsheet ? "Dane z arkusza" : "Dane z PDF"}</h2></div>{sourceUrl ? <a href={sourceUrl} target="_blank" rel="noreferrer" className="inline-flex min-h-11 items-center gap-2 rounded-lg px-2 text-sm font-bold text-brand-strong hover:bg-brand-soft"><ExternalLink aria-hidden="true" size={17} />Otwórz PDF</a> : null}</div><div className="mt-3 space-y-3">{sources.map((source) => <article key={source.id} className="rounded-lg border border-border bg-[#faf9f5] p-3"><p className="text-xs font-bold uppercase text-muted-foreground">{source.section} · wiersz źródłowy</p><dl className="mt-2 divide-y divide-border text-sm"><Row label="Nazwa" value={source.rawName} /><Row label="Adres" value={source.rawAddress ?? "Brak w źródle"} /><Row label="Telefon" value={source.rawPhone ?? "Brak w źródle"} /><Row label="E-mail / WWW" value={[source.rawEmail, source.rawWebsite].filter(Boolean).join(" · ") || "Brak w źródle"} /><Row label="Godziny" value={source.rawOpeningHours ?? "Brak w źródle"} /><Row label="Godziny przyjęć" value={source.rawAdmissionHours ?? "Brak w źródle"} /><Row label="Forma pomocy" value={source.rawAssistanceDescription ?? "Brak w źródle"} /></dl><details className="mt-2"><summary className="min-h-11 cursor-pointer py-2 text-sm font-bold text-brand-strong">Pokaż pełny zapis źródłowy</summary><pre className="overflow-x-auto whitespace-pre-wrap rounded-md bg-white p-3 font-sans text-sm">{source.rawText}</pre></details></article>)}</div></section>;
}

function CompletenessPanel({ checks, ready }: { checks: ReturnType<typeof getVerificationCompleteness>["checks"]; ready: boolean }) {
  return <section className="rounded-lg border border-border bg-white p-4"><h2 className="font-bold">Status kompletności</h2><ul className="mt-3 space-y-2">{checks.map((check) => { const Icon = check.state === "complete" ? Check : check.state === "missing" ? X : CircleHelp; return <li key={check.key} className="flex items-center justify-between gap-3 text-sm"><span>{check.label}</span><span className={`inline-flex items-center gap-1 font-bold ${check.state === "complete" ? "text-brand-strong" : check.state === "missing" ? "text-urgent" : "text-muted-foreground"}`}><Icon aria-hidden="true" size={16} />{check.state === "complete" ? "Potwierdzone" : check.state === "missing" ? "Brakuje" : check.state === "unknown" ? "Brak danych" : "Opcjonalne"}</span></li>; })}</ul><p className={`mt-4 rounded-md p-3 text-sm font-bold ${ready ? "bg-brand-soft text-brand-strong" : "bg-[#efede7] text-muted-foreground"}`}>{ready ? "Minimalne dane są gotowe do publikacji." : "Miejsce nie jest jeszcze gotowe do publikacji."}</p></section>;
}
...
function AccommodationReview({ accommodation, admissionHours }: { accommodation: NonNullable<NonNullable<Awaited<ReturnType<typeof getAdminPlace>>>["accommodation"]>; admissionHours: ReturnType<typeof scheduleFromRows> }) {
  return <section className="rounded-lg border border-border bg-white p-4 sm:p-5"><h2 className="text-xl font-bold">Nocleg — kontrola przed publikacją</h2><dl className="mt-3 divide-y divide-border text-sm"><Row label="Typ" value={accommodationTypeLabels[accommodation.type]} /><Row label="Grupy docelowe" value={accommodation.targetGroups.join(" · ") || "Brak danych"} /><Row label="Godziny przyjęć" value={admissionHours.map((day) => day.status === "OPEN" ? `${weekdayLabels[day.weekday]}: ${day.periods.map((period) => `${period.opensAt}-${period.closesAt}`).join(", ")}` : null).filter(Boolean).join("; ") || accommodation.admissionHoursDescription || "Brak danych"} /><Row label="Trzeźwość" value={sobrietyPolicyLabels[accommodation.sobrietyPolicy]} /><Row label="Zwierzęta" value={petPolicyLabels[accommodation.petPolicy]} /><Row label="Aktualna dostępność" value={accommodation.availabilityLabel ?? "Brak danych — potwierdź telefonicznie"} /></dl>{accommodation.capacityGroups.length ? <div className="mt-3"><h3 className="text-sm font-bold">Pule miejsc</h3><ul className="mt-2 divide-y divide-border rounded-md border border-border">{accommodation.capacityGroups.map((group) => <li key={group.id} className="flex justify-between gap-3 px-3 py-2 text-sm"><span>{group.label}</span><strong>{group.totalBeds ?? "Brak danych"} wszystkich · {group.availableBeds ?? "Brak danych"} wolnych</strong></li>)}</ul></div> : null}</section>;
}
