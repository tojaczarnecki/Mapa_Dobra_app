  ...
  return (
    <div className="space-y-5">
      ...
      {filtered.length ? <ol className="space-y-2">{filtered.map((item) => {
        const flags = [item.issueType !== "NEW_PLACE" ? typeLabels[item.issueType] : null, item.missingCoordinates ? "Brak współrzędnych" : null, item.missingHours ? "Brak potwierdzonych godzin" : null, item.missingPhone ? "Brak telefonu" : null].filter(Boolean) as string[];
