...
const inputClass = "mt-1 min-h-11 w-full rounded-lg border border-border bg-white px-3 py-2 text-sm font-normal outline-none transition focus:border-brand focus:ring-2 focus:ring-brand/25";
const sectionClass = "border-t border-border pt-6";

function TokenInput({ name, initialValues = [], label, placeholder }: { name: string; initialValues?: string[]; label: string; placeholder: string }) {
  const [values, setValues] = useState(initialValues);
  const [draft, setDraft] = useState("");
  function add() { const value = draft.trim().replace(/,/gu, ""); if (value && !values.includes(value)) setValues([...values, value]); setDraft(""); }
  return <div><label className="text-sm font-bold">{label}<input value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === ",") { event.preventDefault(); add(); } }} onBlur={add} placeholder={placeholder} className={inputClass} /></label><input type="hidden" name={name} value={values.join(", ")} />{values.length ? <div className="mt-2 flex flex-wrap gap-1.5">{values.map((value) => <span key={value} className="inline-flex min-h-8 items-center gap-1 rounded-full border border-brand/25 bg-brand-soft px-2.5 text-xs font-semibold text-brand-strong">{value}<button type="button" aria-label={`Usuń ${value}`} onClick={() => setValues(values.filter((item) => item !== value))}><X size={14} aria-hidden="true" /></button></span>)}</div> : null}</div>;
}
