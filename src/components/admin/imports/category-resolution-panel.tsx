...
export function CategoryResolutionPanel({ candidateId, sourceValue, tokens, reanalysis, effectiveState, effectiveCategoryIds, activeCategories, currentDecision, canEdit }: {
  ...
  const initialPrimary = currentDecision?.primaryCategoryId ?? (effectiveState === "AUTO_SINGLE" ? effectiveCategoryIds[0] ?? "" : "");
  const initialSelected = currentDecision?.categories.map((item) => item.categoryId) ?? (effectiveState === "AUTO_SINGLE" ? effectiveCategoryIds : effectiveState === "REQUIRES_REVIEW" ? effectiveCategoryIds : []);
  const [primaryCategoryId, setPrimaryCategoryId] = useState(initialPrimary);
  const [selectedCategoryIds, setSelectedCategoryIds] = useState(initialSelected);
  useEffect(() => { if (state.ok) router.refresh(); }, [router, state]);

  const orderedSelected = useMemo(() => [primaryCategoryId, ...selectedCategoryIds.filter((id) => id !== primaryCategoryId)].filter(Boolean), [primaryCategoryId, selectedCategoryIds]);
  const categoryName = (id: string) => activeCategories.find((category) => category.id === id)?.name ?? "Nieaktywna kategoria";
  const hasExistingDecision = Boolean(currentDecision);
  ...
  return <section className="mt-3 rounded-md border border-border bg-[#faf9f5] p-3" aria-labelledby={`category-decision-${candidateId}`}>
    ...
    {tokens.length ? <div className="mt-3"><p className="text-sm font-bold">Analiza systemu</p><ul className="mt-2 space-y-1 text-sm">{tokens.map((token, index) => <li key={`${token.sourceToken}-${index}`} className="rounded-md border border-border bg-white p-2"><span className="font-semibold">{token.sourceToken}</span>{token.unresolved ? <span className="ml-2 text-urgent">Nie rozpoznano</span> : <span className="ml-2">→ {token.categoryName ?? token.categorySlug}{token.method ? ` · ${token.method}` : ""}</span>}{token.warnings.length ? <span className="block text-xs text-muted-foreground">{token.warnings.join(", ")}</span> : null}</li>)}</ul></div> : <p className="mt-3 text-sm text-muted-foreground">Brak zapisanej analizy tokenów.</p>}
    {reanalysis ? <div className="mt-3 rounded-md border border-brand/25 bg-brand-soft/30 p-3 text-sm"><p className="font-bold">Ponowna analiza</p><p className="mt-1">{reanalysis.status}: {reanalysis.matchedCategoryNames.join(", ") || "brak dopasowań"}</p>{reanalysis.unresolvedTokens.length ? <p className="mt-1 text-urgent">Nierozpoznano: {reanalysis.unresolvedTokens.join(", ")}</p> : null}</div> : null}
    {currentDecision ? <div className="mt-3 border-t border-border pt-3 text-sm"><p className="font-bold">Aktualna decyzja</p><p className="mt-1">Kategoria główna: {categoryName(currentDecision.primaryCategoryId)}</p><p className="mt-1">Kategorie dodatkowe: {currentDecision.categories.filter((item) => item.categoryId !== currentDecision.primaryCategoryId).map((item) => item.name).join(", ") || "brak"}</p>{currentDecision.resolvedBy ? <p className="mt-1 text-muted-foreground">{currentDecision.resolvedBy}{currentDecision.resolvedAt ? ` · ${new Intl.DateTimeFormat("pl-PL", { dateStyle: "medium", timeStyle: "short" }).format(new Date(currentDecision.resolvedAt))}` : ""}</p> : null}{currentDecision.note ? <p className="mt-1 text-muted-foreground">Notatka: {currentDecision.note}</p> : null}</div> : null}
    {canEdit ? <form action={action} className="mt-4 space-y-3 border-t border-border pt-3"><input type="hidden" name="candidateId" value={candidateId} /><input type="hidden" name="primaryCategoryId" value={primaryCategoryId} /><input type="hidden" name="selectedCategoryIds" value={JSON.stringify(orderedSelected)} /><label className="block text-sm font-bold">Kategoria główna<select value={primaryCategoryId} onChange={(event) => { const value = event.target.value; setPrimaryCategoryId(value); setSelectedCategoryIds((current) => value ? [value, ...current.filter((id) => id !== value)] : current); }} className="mt-1 min-h-11 w-full rounded-lg border border-border bg-white px-3 font-normal" required><option value="">Wybierz kategorię główną</option>{activeCategories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label><fieldset><legend className="text-sm font-bold">Dodatkowe kategorie</legend><div className="mt-1 grid gap-1.5 sm:grid-cols-2">{activeCategories.map((category) => { const selected = orderedSelected.includes(category.id); const isPrimary = category.id === primaryCategoryId; return <label key={category.id} className="flex min-h-11 items-center gap-2 rounded-md border border-border px-2 text-sm"><input type="checkbox" checked={selected} disabled={isPrimary} onChange={(event) => setSelectedCategoryIds((current) => event.target.checked ? [...current, category.id] : current.filter((id) => id !== category.id))} className="h-5 w-5 accent-[#13ad87]" />{category.name}{isPrimary ? <span className="text-xs font-bold text-brand-strong">Główna</span> : null}</label>; })}</div></fieldset><label className="block text-sm font-bold">Notatka <span className="font-normal text-muted-foreground">(opcjonalnie)</span><textarea name="note" maxLength={1000} defaultValue={currentDecision?.note ?? ""} className="mt-1 min-h-20 w-full rounded-lg border border-border bg-white px-3 py-2 font-normal" /></label>{state.message ? <p role={state.ok ? undefined : "alert"} className={`text-sm font-semibold ${state.ok ? "text-brand-strong" : "text-[#8c2d0c]"}`}>{state.message}</p> : null}<SaveButton editing={hasExistingDecision} /></form> : <p className="mt-3 border-t border-border pt-3 text-sm text-muted-foreground">Decyzja kategorii jest tylko do odczytu dla tego rekordu.</p>}
  </section>;
}
