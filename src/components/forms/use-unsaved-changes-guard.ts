...
export function useUnsavedChangesGuard(enabled: boolean) {
  useEffect(() => {
    ...
    const beforeUnload = (event: BeforeUnloadEvent) => { if (!allowUnload) { event.preventDefault(); event.returnValue = message; } };
    const click = (event: MouseEvent) => {
      const link = (event.target as HTMLElement | null)?.closest("a[href]") as HTMLAnchorElement | null;
      if (!link || link.target === "_blank" || link.href === window.location.href) return;
      if (window.confirm(message)) allowUnload = true;
    ...
    const popstate = () => {
      ...
      if (window.confirm(message)) { allowUnload = true; window.location.assign(destination); }
    };
