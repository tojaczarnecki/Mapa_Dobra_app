...
const LocationMap = dynamic(() => import("./help-request-location-map").then((module) => module.HelpRequestLocationMap), { ssr: false });
type LocationMode = "address" | "map";
type Screen = 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8;
type FormState = { emergencyAnswer: EmergencyAnswer; emergencyAnswerSelected: boolean; locationMode: LocationMode; addressText: string; locationDescription: string; latitude?: number; longitude?: number; locationAccuracy?: number; needs: HelpRequestNeed[]; description: string; reporterName: string; reporterPhone: string; reporterEmail: string };
type HelpRequestDraft = Omit<FormState, "reporterName" | "reporterPhone" | "reporterEmail">;
const initialState: FormState = { emergencyAnswer: null, emergencyAnswerSelected: false, locationMode: "address", addressText: "", locationDescription: "", needs: [], description: "", reporterName: "", reporterPhone: "", reporterEmail: "" };
const needOrder = ["OTHER", ...Object.keys(helpRequestNeedLabels).filter((need) => need !== "OTHER")] as HelpRequestNeed[];
const screenForStep: Record<HelpRequestStep, Screen> = { 1: 1, 2: 2, 3: 5, 4: 7 };

function draftStepForScreen(screen: Screen): HelpRequestStep {
  if (screen <= 2) return screen as 1 | 2;
