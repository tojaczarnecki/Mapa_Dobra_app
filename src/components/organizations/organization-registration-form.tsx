...
const draftExcludedFields = new Set(["password", "turnstileToken", "websiteTrap"]);

function readSafeFormDraft(form: HTMLFormElement) {
  return Object.fromEntries(
    Array.from(new FormData(form).entries()).filter(([key]) => !draftExcludedFields.has(key)),
  );
}
...
export function OrganizationRegistrationForm() {
  ...
  const [draftData, setDraftData] = useState<Record<string, FormDataEntryValue>>({});
  const draft = useFormDraft({ formType: "organization-registration", storage: "session", ttlMs: 24 * 60 * 60 * 1000, data: draftData, enabled: !result?.ok });
  useUnsavedChangesGuard(!result?.ok && draft.isDirty);
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setResult(body); setPending(false); if (body.ok) { draft.clear(); event.currentTarget.reset(); setDraftData({}); }
  }
